...
<head>
    ...
    <title>@yield('title', 'Area Cittadino') — GTE Abruzzo</title>
...
<div class="min-h-full">
    ...
    <div class="bg-green-50 border-b border-green-200 px-4 py-2.5">
        <p class="text-sm text-green-800 max-w-5xl mx-auto">{{ session('success') }}</p>
    </div>
    @endif
    @if(session('error'))
    <div class="bg-red-50 border-b border-red-200 px-4 py-2.5">
        <p class="text-sm text-red-800 max-w-5xl mx-auto">{{ session('error') }}</p>
    </div>
    @endif
